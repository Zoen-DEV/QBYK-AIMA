...
def vision(monkeypatch):
    ...
    def _verificar(src, esperado, *, cfg):
        estado["llamadas"].append(esperado)
        visto = estado["lee"](esperado, len(estado["llamadas"])) if estado["lee"] else esperado
        if iqa.coincide(esperado, visto):
        ...
        return iqa.ResultadoQA(ok=False, verificado=True, texto_visto=visto,
                               motivo=f'esperaba "{esperado}" y la imagen dice "{visto}"')
