...
- `visual_identity.py` es el esquema hecho validador, y hace comprobables **tres contratos que
  ...
  **croma, no por contraste**: hueso y lima ácido contrastan 1.03:1 y nadie los confunde. Son
  ...
  nombre una sans neutra de interfaz (`FAMILIAS_UI_PROHIBIDAS` — a un titular del 9-16% del alto
  devuelve el look de «caption pegado sobre una foto», justo lo que el prompt de extracción ya
  advertía y el validador dejaba pasar). Que la familia no se declare de display
  (`MARCAS_DISPLAY`) y que la secundaria pida `regular weight` o `mixed case` son **reparos**:
  pueden ser decisiones legítimas. Ojo con el efecto secundario, que es real: `validar` corre al
